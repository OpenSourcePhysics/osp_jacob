...
public class RandomForce extends ComponentModifier
{
   public static final String TYPE = "randomforce";
...
//------------------------------------------------------------------------------
    ...
    public void preIntegrationModify( long time )
    {
        ...
        if (getComponent() instanceof Particle)
        {
			if (((Particle)getComponent()).isFixed())
				loc = new Point2d( cloc );
			else
				loc = getComponent().getLocation();
		}
		else
		{
			loc = new Point2d( cloc );
