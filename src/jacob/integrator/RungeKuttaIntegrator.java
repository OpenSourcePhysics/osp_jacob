...
public class RungeKuttaIntegrator extends ModIntegrator
{
    ...
    private double elasticityConst;
    ...
    private double a, b, m, h;

    private double Avx, Avy, Ax, Ay;
    private double Bvx, Bvy, Bx, By;
    private double Cvx, Cvy, Cx, Cy;
    private double Dvx, Dvy, Dx, Dy;
...
//FIXME VV 13.10.02
    public void integrate( Scene scene )
    {
        ...
        elasticityConst = scene.getProperties().elasticityConst.getValue();
        inertiaConst    = scene.getProperties().inertiaConst.getValue();
...
// Runge Kutta 2 order  Integration
            ...
            Bvx = (-a * (vx + 0.5*h*Avx) - b * (x + 0.5*h*Ax) + Fx)/m;
            Bvy = (-a * (vy + 0.5*h*Avy) - b * (y + 0.5*h*Ay) + Fy)/m;
		    Bx  = vx + 0.5*h*Avx;
            By  = vy + 0.5*h*Avy;

            Cvx = (-a * (vx + 0.5*h*Bvx) - b * (x + 0.5*h*Bx) + Fx)/m;
			Cvy = (-a * (vy + 0.5*h*Bvy) - b * (y + 0.5*h*By) + Fy)/m;
		    Cx  = vx + 0.5*h*Bvx;
            Cy  = vy + 0.5*h*Bvy;

            Dvx = (-a * (vx + h*Cvx) - b * (x + h*Bx) + Fx)/m;
			Dvy = (-a * (vy + h*Cvy) - b * (y + h*By) + Fy)/m;
		    Dx  = vx + h*Cvx;
            Dy  = vy + h*Cvy;

            loc.x += h/6*(Ax + 2*Bx + 2*Cx + Dx);
            loc.y += h/6*(Ay + 2*By + 2*Cy + Dy);

            vx += h/6*(Avx + 2*Bvx + 2*Cvx + Dvx);
            vy += h/6*(Avy + 2*Bvy + 2*Cvy + Dvy);
