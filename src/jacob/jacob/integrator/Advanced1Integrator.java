...
package jacob.jacob.integrator;

import jacob.geometry.*;
import jacob.integrator.ModIntegrator;
import jacob.scene.*;


public class Advanced1Integrator extends ModIntegrator
{
...
//by Dragan Simeonov 03.10.02
    protected void integrate0( Scene scene )
    {
        int nobjects = scene.getView().countChildren();
       	Particle mparticles[] = new Particle[nobjects];

        for (int i=0; i<nobjects; i++)
		{
			SceneComponent E = scene.getView().getChildAt(i);
			ParticleIterator iterator0 = new ParticleIterator(E.createIterator());
			mparticles[i] = new Particle(scene, 0);
			mparticles[i].move(new Point2d(E.getLocation().x, E.getLocation().y));

			while ( !iterator0.isDone() )
			{
				mparticles[i].setCharge( mparticles[i].getCharge() +
										 iterator0.getCurrentParticle().getCharge());
				iterator0.next();
			}
		}
